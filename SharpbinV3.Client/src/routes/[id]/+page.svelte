<script lang="ts">
	...
	async function reactToPaste(target: 1 | 2) {
		if (!data?.paste || reactingToPaste || $user?.uuid === data.paste.author?.uuid) return;
			...
			>
				{#if !isBinary}
					<div class="flex items-center gap-2 text-sm text-neutral-300">
						...
						<button
							type="button"
							class="ml-4 rounded border px-2 py-0.5 text-sm transition-colors disabled:cursor-not-allowed! disabled:opacity-60 {data
							...
							disabled={reactingToPaste || !$user || $user?.uuid === data.paste.author?.uuid}
							aria-disabled={reactingToPaste || !$user || $user?.uuid === data.paste.author?.uuid}
						>
						...
						<button
							type="button"
							class="rounded border px-2 py-0.5 text-sm transition-colors disabled:cursor-not-allowed! disabled:opacity-60 {data

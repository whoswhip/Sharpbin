<script lang="ts">
	...
	interface Props {
		...
		placeholder?: string;
		searchable?: boolean;
		displayValue?: (val: string | number) => string;
		variant?: string;
	}
	...
	let dropdownListElement = $state<HTMLDivElement | undefined>(undefined);
	let dropdownMaxHeight = $state('12rem');
...
<div bind:this={dropdownElement} class="relative mb-2 w-full" class:dropdown-sm={variant === 'sm'}>
	...
	{#if isOpen}
		...
		>
			...
			{/if}
			<div
				class="scrollbar-thin scrollbar-thumb-neutral-600 scrollbar-track-neutral-700"
				style="overflow-y: auto; max-height: calc({dropdownMaxHeight} - 3rem);"
			>

    ...
    ) : ControllerBase
    {
        ...
        )
        {
            ...
            if (!await _pasteService.ValidateVisibility(visibility))
                return BadRequest(
                    new { message = "Invalid visibility level. Must be between 0 and 2." }
                );
            ...
            if (!await _pasteService.ValidateTitle(title))
                return BadRequest(
                    new
                    {
                        message = $"Invalid title. Must be less than {_pasteSettings.MaxTitleLength} characters.",
                    }
                );
            ...
            if (
                System.Text.Encoding.UTF8.GetByteCount(content) > _pasteSettings.MaxPasteSizeInBytes
            )
                return BadRequest(
                    new
                    {
                        message = $"Paste size exceeds the maximum allowed size of {_pasteSettings.MaxPasteSizeInBytes} bytes.",
                    }
                );

            if (
                _pasteSettings.RequiresVerfication
                && !await _verificationService.VerifyAsync(
                    new VerificationContext { Token = token, Ip = HttpContext.GetRequestIP() }
                )
            )
                return BadRequest(new { message = "Verification failed." });

            var user = await _authService.GetUserFromHttpContext(HttpContext);
            Paste paste = await _pasteService.Create(
                user,
                content,
                title,
                syntax,
                visibility,
                expiresAt,
                _pasteSettings.EnablePasteCompression
            );
            return Ok(
                new
                {
        ...
        public async Task<IActionResult> GetPasteByID(string id)
        {
            ...
            return Ok(
                new
                {
                    ...
                    paste.ExpiresAt,
                    paste.EditedAt,
                    Author = paste.User != null && paste.User.Visibility == 0
                        ? new
                        {
                            paste.User.UID,
                            paste.User.UUID,
        ...
        public async Task<IActionResult> EditPaste(string id)
        {
            ...
            if (user.Roles.Contains(255) && !user.TotpEnabled && _authSettings.Admins_Require_2FA)
                return StatusCode(
                    403,
                    new { success = false, message = "2FA is required to perform this action." }
                );
            ...
            if (!result)
                return StatusCode(
                    500,
                    new { success = false, message = "An error occurred while editing the paste." }
                );
        ...
        [Route("{id}/modify")]
        [Authorize(Policy = "AuthAndNotBanned")]
        public async Task<IActionResult> ModifyPasteMetadata(
            string id,
            [FromBody] UpdatePasteDto request
        )
        {
            var paste = await _pasteService.Get(id);
            if (paste == null)

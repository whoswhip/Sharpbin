...
namespace SharpbinV3.Server.Controllers
{
    ...
    ) : ControllerBase
    {
        ...
        [HttpPatch("uuid/{uuid}")]
        [Authorize]
        public async Task<IActionResult> UpdateByUUID(
            Guid uuid,
        ...
        )
        {
            ...
            if (user.Roles.Contains(255) && !jwtUser.Roles.Contains(255))
                return Forbid();
            if (
                jwtUser.Roles.Contains(255)
                && !jwtUser.TotpEnabled
                && _authSettings.Admins_Require_2FA
            )
                return Forbid();
        ...
        public async Task<IActionResult> DeleteByUUID(Guid uuid, [FromBody] DeleteUserDto dto)
        {
            ...
            if (jwtUser.TotpEnabled)
            {
                if (
                    ...
                    || !await _totp.VerifyAsync(
                        new VerificationContext { UserUUID = jwtUser.UUID, Code = dto.TotpCode }
                    )
                )
                {
                    return Unauthorized(new { message = "Invalid TOTP code." });
        ...
        private async Task<object> BuildUserResponse(User user, int page = 1)
        {
            var isAuthenticatedUser =
                HttpContext.User?.FindFirst("UUID")?.Value == user.UUID.ToString();
